...
# Threads for each qchem job process
procthreads=`expr $OMP_NUM_THREADS / 2`
export QCTHREADS="$procthreads"
...
for i in $( seq ${1} ${2} ); do
    ...
    if [ -e "hcn/hcn.input" ]; then
        ...
        # Run QChem
        qchem -nt $QCTHREADS hcn.input > hcn.output &
    cd ../
    fi
    ...
    if [ -e "fhf/fhf.input" ]; then
        ...
        # Run QChem
        qchem -nt $QCTHREADS fhf.input > fhf.output &
    cd ../
    fi
